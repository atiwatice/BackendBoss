...
//	List<?> findTaskPercentByTaskOwnerId(int taskOwnerId);
	...
	@Query(value ="select task.task_id as task_id,task.task_name as task_name, NVL(sum(percent),0) as total_percent from task left outer join sub_task on task.task_id=sub_task.task_id where task.task_owner_id = ?1 group by task.task_id,task.task_name",nativeQuery = true)
